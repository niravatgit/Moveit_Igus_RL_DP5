...
class ClickAndHoldApp:
    ...
    def jog(self, event, axis, direction):
        cur_position = float(self.axis_controller.axes[axis].getPosition())
        desired_postion =  int(cur_position + direction*10)
        self.axis_controller.axes[axis].profile_pos_mode(desired_postion, 100, 100)#(axis, desired_postion)
        print(f"Started jogging axis ", axis , " from current postionn= ", cur_position, " To desired position = " , desired_postion)
