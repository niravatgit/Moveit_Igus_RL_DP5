...
def normals_callback(msg):
    global normals_data
    normals_data = pcl.PointCloud()
    pcl.PointCloud.fromROSMsg(msg, normals_data)
    process_normals()

def process_normals():
    global normals_data

    # Extract the XYZ coordinates from the PointCloud
    xyz = np.asarray(normals_data)

    # Assuming normals_data has shape (num_points, 3) where each row is a 3D normal vector
    x, y, z = xyz.T

    # Visualize the normals vectors
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    ax.quiver3D(np.zeros_like(x), np.zeros_like(y), np.zeros_like(z), x, y, z, length=0.1, normalize=True)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
    plt.show()
...
def main():
    rospy.init_node('normals_subscriber', anonymous=True)
    rospy.Subscriber('/cloud_normals_something', PointCloud2, normals_callback)
    rospy.spin()
...
cloud_pub = rospy.Publisher('segmented_point_cloud', PointCloud2, queue_size=1)
normal_pub = rospy.Publisher('normals', PointCloud2, queue_size=1)

def cloud_callback(cloud_msg):
    global normal_pub, cloud_pub
    ...
    o3d_cloud.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamKNN(knn=50))

    # Get the normals as a numpy array
    normals = np.asarray(o3d_cloud.normals)static_transforms = []
            for i, pose_goal in enumerate(points_to_follow):
    ...
    # Publish the original segmented point cloud
...
def visualize_trajectory(plan):
    ...
    if display_trajectory_publisher is not None:
        display_trajectory = moveit_msgs.msg.Displastatic_transforms = []
            for i, pose_goal in enumerate(points_to_follow):
                translation = [pose_goal.position.x, pose_goal.position.y, pose_goal.position.z]
                rotation = [pose_goal.orientation.x, pose_goal.orientation.y, pose_goal.orientation.z, pose_goal.orientation.w]
            ...
            for static_transform in static_transforms:
                ...
                t.transform.rotation.w = static_transform['rotation'][3]
                t_list.append(t)
            static_broadcaster.sendTransform(t_list)yTrajectory()
...
def point_cloud_callback(cloud_msg):
    try:
        ...
        for i, pose_goal in enumerate(normal_points):
            ...
            static_transforms.append({
                'parent_frame_id': 'royale_cam0_optical_frame',
                'child_frame_id': 'frame_{}'.format(i),
                'translation': translation,
                'rotation': rotation
            })
        ...
        # new_waypoints = []
        # for i, normal in enumerate(normal_vectors):base_link
