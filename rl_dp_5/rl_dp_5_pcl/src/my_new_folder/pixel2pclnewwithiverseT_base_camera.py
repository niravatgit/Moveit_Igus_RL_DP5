...
class RealRobotValidation:
    def __init__(self):
        ...
        # Pre-compute inverse of T_base_marker
        self.T_marker_base = np.linalg.inv(self.T_base_marker)

        # Initialize transformation matrix (will be filled in dynamically)
        self.T_camera_marker = None
    ...
    def lookup_marker_transform(self):
        ...
        try:
            ...
            # Store inverse (camera to marker transform)
            self.T_camera_marker = np.linalg.inv(T_marker_camera)
            rospy.loginfo(f"\n=== T_camera_marker ===\n{self.T_camera_marker}")
    ...
    def transform_to_robot_frame(self, points):
        """Transform points from camera frame to robot base frame using:
           P_base = (T_base_marker^-1) × T_camera_marker × P_camera
        """
        if self.T_camera_marker is None:
        ...
        # Apply transformation: P_base = T_marker_base × T_camera_marker × P_camera
        points_robot_homog = np.matmul(self.T_camera_marker, points_homog.T)
        points_robot_homog = np.matmul(self.T_marker_base, points_robot_homog)
